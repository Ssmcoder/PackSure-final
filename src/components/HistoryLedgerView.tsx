...
export const HistoryLedgerView: React.FC<HistoryLedgerViewProps> = ({ samples = [], onNavigateScan }) => {
      ...
      <div className="bg-white rounded-xl shadow-xs border border-[#dce9ff] overflow-hidden">
        {filteredSamples.length === 0 ? (
          <div className="p-12 flex flex-col items-center justify-center text-center space-y-3">
            <div className="w-14 h-14 rounded-full bg-[#e5eeff] flex items-center justify-center text-[#76777d]">
              <span className="material-symbols-outlined text-[28px]">folder_off</span>
            </div>
            <div className="text-base font-semibold text-[#0b1c30]">
              No Inspection Dossiers Recorded
            </div>
            <p className="text-xs text-[#76777d] max-w-md">
              The ledger is currently blank. Upload package labels or perform camera scans in the Verification Terminal to record dossiers.
            </p>
            {onNavigateScan && (
              <button
                onClick={onNavigateScan}
                className="mt-2 px-4 py-2 rounded-lg bg-[#000000] text-white text-xs font-semibold hover:opacity-90 transition-opacity"
              >
